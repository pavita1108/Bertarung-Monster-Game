    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            for (String[] line : lines) {
                ...
                if (moveType.equals("STATUS")) {
                    ...
                    StatusMove a = new StatusMove(id, name, elType, accuracy, priority, ammunition, target, moveStatus,
                            stat);
            ...
            for (String[] line1 : lines1) {
                ...
                Stats basestats = new Stats(stats.get(0), stats.get(1), stats.get(2), stats.get(3), stats.get(4),
                        stats.get(5));
            ...
            CSVReader reader2 = new CSVReader(
                    new File(Main.class.getResource("configs/element-type-effectivity-chart.csv").toURI()), ";");
        ...
        while (menu != 5) {
            if (menu == 1) {
                ...
                while (!p1Lose && !p2Lose) {
                    ...
                    for (int i = 0; i < p1.getJumlahMonster(); i++) {
                        if (p1.getListMonster().get(i).getIsSleep()) {
                            p1.getListMonster().get(i)
                                    .setSleepCounter(p1.getListMonster().get(i).getSleepCounter() - 1);
                    ...
                    for (int i = 0; i < p2.getJumlahMonster(); i++) {
                        if (p2.getListMonster().get(i).getIsSleep()) {
                            p2.getListMonster().get(i)
                                    .setSleepCounter(p2.getListMonster().get(i).getSleepCounter() - 1);
                    ...
                    while (!turnFinished) {
                        ...
                        System.out.println("3. View current monster status!!!");
                        ...
                        if (select1 == 1) {
                            ...
                            if (p1ActiveMons.getIsSleep()) {
                                ...
                                System.out.println(
                                        "Monster yang aktif sedang sleep, tidak bisa melakukan move, sekarang giliran Player lain turn");
                            } else {
                                // ngecek paralyzed
                                if (p1ActiveMons.getIsParalyze()) {
                                    // ngerandom 25% bisa move apa engga, nilai true falsenya masukin ke variable
                                    // p1BisaMove
                    ...
                    if ((p1PilihanMoveValid == true) && (p2PilihanMoveValid == true)) {
                        ...
                        if (p1ActiveMons.getMoves().get(p1PilMove).getPriority() == p2ActiveMons.getMoves()
                                .get(p2PilMove).getPriority()) {
                            System.out.println("AAAAAAAAAAAAAAAAAAAAAAAAAAA");
                            if (p1ActiveMons.getStats().getSpeed() > p2ActiveMons.getStats().getSpeed()) {
                                System.out.println(p1ActiveMons.getName() + "menyerang duluan");
                                if (p1ActiveMons.getMoves().get(p1PilMove) instanceof StatusMove) {
                                    p1ActiveMons.getMoves().get(p1PilMove).applyEffect(p1ActiveMons, p2ActiveMons,
                                            listEffectivity);
                                    Main.applyStatusMove(p2ActiveMons, p1PilMove);
                                    if (!p2ActiveMons.getIsSleep()) {
                                        System.out.println(p2ActiveMons.getName() + "menyerang balik menggunakan"
                                                + p2ActiveMons.getMoves().get(p2PilMove));
                                        if (p2ActiveMons.getMoves().get(p2PilMove) instanceof StatusMove) {
                                            p2ActiveMons.getMoves().get(p2PilMove).applyEffect(p1ActiveMons,
                                                    p2ActiveMons, listEffectivity);
                                            Main.applyStatusMove(p1ActiveMons, p2PilMove);
                                        } else if (p2ActiveMons.getMoves().get(p2PilMove) instanceof SpecialMove) {
                                            p2ActiveMons.getMoves().get(p2PilMove).applyEffect(p2ActiveMons,
                                                    p1ActiveMons, listEffectivity);
                                            Main.applyStatusMove(p1ActiveMons, p2PilMove);
                                        } else if (p2ActiveMons.getMoves().get(p2PilMove) instanceof NormalMove) {
                                            p2ActiveMons.getMoves().get(p2PilMove).applyEffect(p2ActiveMons,
                                                    p1ActiveMons, listEffectivity);
                                            Main.applyStatusMove(p1ActiveMons, p2PilMove);
                                        }
                                    } else if (p1ActiveMons.getMoves().get(p1PilMove) instanceof SpecialMove) {
                                        p1ActiveMons.getMoves().get(p1PilMove).applyEffect(p1ActiveMons, p2ActiveMons,
                                                listEffectivity);
                                        Main.applyStatusMove(p2ActiveMons, p1PilMove);
                                    } else if (p1ActiveMons.getMoves().get(p1PilMove) instanceof NormalMove) {
                                        p1ActiveMons.getMoves().get(p1PilMove).applyEffect(p1ActiveMons, p2ActiveMons,
                                                listEffectivity);
                                        Main.applyStatusMove(p2ActiveMons, p1PilMove);
                                    }
                                } else if (p1ActiveMons.getStats().getSpeed() < p2ActiveMons.getStats().getSpeed()) {
                                    System.out.println(p2ActiveMons.getName() + "menyerang duluan");
                                    /* aaaaaa */
                                    if (p2ActiveMons.getMoves().get(p2PilMove) instanceof StatusMove) {
                                        p2ActiveMons.getMoves().get(p2PilMove).applyEffect(p2ActiveMons, p1ActiveMons,
                                                listEffectivity);
                                        Main.applyStatusMove(p1ActiveMons, p2PilMove);
                                        if (!p2ActiveMons.getIsSleep()) {
                                            System.out.println(p1ActiveMons.getName() + "menyerang balik menggunakan"
                                                    + p1ActiveMons.getMoves().get(p2PilMove));
                                            if (p1ActiveMons.getMoves().get(p1PilMove) instanceof StatusMove) {
                                                p1ActiveMons.getMoves().get(p1PilMove).applyEffect(p1ActiveMons,
                                                        p2ActiveMons, listEffectivity);
                                                Main.applyStatusMove(p1ActiveMons, p2PilMove);
                                            } else if (p1ActiveMons.getMoves().get(p1PilMove) instanceof SpecialMove) {
                                                p1ActiveMons.getMoves().get(p1PilMove).applyEffect(p1ActiveMons,
                                ...
                                } else if (p1ActiveMons.getMoves().get(p1PilMove).getPriority() > p2ActiveMons
                                        .getMoves().get(p2PilMove).getPriority()) {
                                    // eksekusi move p1
                                    if (p1ActiveMons.getMoves().get(p1PilMove) instanceof StatusMove) {
                                        //
                                        if (!p2ActiveMons.getIsBurn() && !p2ActiveMons.getIsSleep()
                                                && !p2ActiveMons.getIsParalyze() && !p2ActiveMons.getIsPoison()) {
                                ...
                                if (p2ActiveMons.getIsSleep()) {
                                    System.out.println(
                                            "Monster ini baru saja terkena efek sleep dari move p1 sebelumnya");
                                } else if (p2ActiveMons.getIsParalyze()) {
                                    System.out.println(
                                            "Monster ini baru saha terkena efek paralyze dari move p1 sebelumnya");
                            ...
                            // kalo p1 doang yang move, p2 engga (karena cuma ngeswitch pas milih turn, atau
                            // karena efek stats condition)
                            ...
                            // kalo p2 doang yang move, p1 engga (karena cuma ngeswitch pas milih turn, atau
                            // karena efek stats condition)
                ...
                System.out.println(
                        "Permainan dimainkan oleh 2 player. Masing-masing player akan mendapatkan 6 monster yang diberikan secara acak.");
            } else if (menu == 3) {
                if (gameOn = false) {
